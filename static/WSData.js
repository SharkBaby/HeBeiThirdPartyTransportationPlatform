...
+function (ns) {
    ...
    ns.Subcmd = {
        ...
        SUBCMD_WARN_ADPT_OVERSPEED: 324,          // 服务器主动上报超速报警信息
        SUBCMD_WARN_ADPT_NIGHTABNORMAL: 325,          // 服务器主动上报2-5报警信息
        SUBCMD_WARN_ADPT_FOURHOURS: 326,          // 服务器主动上报4小时报警信息
        SUBCMD_ENABLE_USERWARN_PUSH: 1002,
        ...
        GOV_VEHICLE_CANCEL_TREACK: 47, //向浩：车辆监管取消跟踪
        ...
        SUBCMD_SAVE_SYSCONFIG: 320,                 //系统配置
        SUBCMD_SEND_EMAIL_EXPORT: 321,              //发送报表邮件
        SUBCMD_UPDATE_COMPANY: 322,                  //修改企业
    };
                ...
                (function (type) {
                    ...
                    proto['set' + action] = function (value) {
                        if (this[type].isEncode) {
                            value = Gov.Encrypt.encoderGB2312(value);
                        }
...
+function (ns) {
    ns.DataBag = {
        ...
        Save_sysconfig: {
            command: ns.Command.CMD_ADMIN | ns.Command.CMD_RECORDSEHR | ns.Command.CMD_RESPONSE,
            subcmd: ns.Subcmd.SUBCMD_SAVE_SYSCONFIG,
            struct: ns.Struct.Save_sysconfig,
            response: ns.Struct.Header
        },
        Send_EmailExport: {
            command: ns.Command.CMD_ADMIN | ns.Command.CMD_RECORDSEHR | ns.Command.CMD_RESPONSE,
            subcmd: ns.Subcmd.SUBCMD_SEND_EMAIL_EXPORT,
            struct: ns.Struct.Send_EmailExport,
            response: ns.Struct.Header
        },
        UpdateCompany: {
            command: ns.Command.CMD_ADMIN | ns.Command.CMD_RECORDSEHR | ns.Command.CMD_RESPONSE,
            subcmd: ns.Subcmd.SUBCMD_UPDATE_COMPANY,
            struct: ns.Struct.UpdateCompany,
            response: ns.Struct.Header
        },
    }
