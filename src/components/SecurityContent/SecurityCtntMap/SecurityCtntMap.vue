...
import "echarts/map/js/china.js"; //第一步：加载某一个省份的地图需要先加载中国的地图
import "echarts/map/js/province/hebei.js"; //第二步：加载河北省地图
...
export default {
  ...
  data() {
    return {
      ...
      getVehicleSpreadDataUrl:
        commonConfig.rootUrl + commonConfig.GetVehicleSpreadData, // 车辆分布页面上的地图点击事件接口
      getOrgOnlineUrl:
        commonConfig.rootUrl + commonConfig.GetLevelPlatDataDetail, // 获得Regulation组件的数据
      mapTypeVal: {
        // 定义是哪一个页面调用该地图模块
  ...
  watch: {
    mapItem: function(val) {
      ...
      echarts.init(document.getElementById("home_security_map")).dispose();//销毁前一个实例
      this.drawSecCtntMap();
    }
  },
  mounted() {
  ...
  methods: {
      ...
      axiosRequest.axiosGet(this.getVehicleSpreadDataUrl, {
        ...
        .then(function(response) {
          ...
          if (response.data[0].VehicleCnt !== 0)
            VehicleAllCnt = response.data[0].VehicleCnt; // 获得当前省份或者地区的所有入网车辆
          for (let vhcleItem of response.data) {
            specificCityArr.push({
              ...
              Rate:
                VehicleAllCnt == 0
                  ? "0.00"
                  : (vhcleItem.VehicleCnt / VehicleAllCnt * 100).toFixed(2) +
                    "%",
      ...
      if (self.mapItemEcharts.mapType == self.mapTypeVal.VehicleDistibution) {
        ...
        myChart.on("mouseover", function(params) {
          const seriesdataTemp =
            self.mapItemEcharts.allVehicleData[params.dataIndex];
          const pieRslt = {
            ...
            seriesdata: [
              ...
              {
                value:
                  seriesdataTemp.HeavyTotalCount +
                  seriesdataTemp.TractorTotalCount,
                name: "重货、半挂牵引车"
              }
            ]
          };
        ...
        myChart.on("mouseup", function(params) {
          self.getVehicleSpecificCity(
            self.mapItemEcharts.sourceData[params.dataIndex].ZoneID
          );
        });
      } else if (self.mapItemEcharts.mapType == self.mapTypeVal.Regulation) {
        //处理mouseup 事件来改变右边的信息
        myChart.on("mouseup", function(params) {
          self.getRegulationSpecificCity(
            self.mapItemEcharts.sourceData[params.dataIndex].ZoneID
          );
        });
      }
    ...
    getChinaMapOption: function() {
      return {
        ...
        visualMap: this.mapItemEcharts.visualMap,
        legend: this.mapItemEcharts.legend,
        series: this.mapItemEcharts.seriesData,
        geo: this.mapItemEcharts.geo
        // series: [{
        //   name: '省级地域分布',
        //   type: 'map',
        //   geoCoord: geoCoord,
        ...
        //     }
        //   },
        //   legend: this.mapItemEcharts.chart_map.legend,
        //   color: this.mapItemEcharts.chart_map.color,
        //   selectedMode: 'single',
        //   itemStyle: {
        //     normal: {
